...
func (suite *E2ETestSuite) makeRequest(method, path string, payload interface{}) (*http.Response, []byte, error) {
	...
	req, err := http.NewRequestWithContext(context.Background(), method, suite.baseURL+path, body)
	if err != nil {
		return nil, nil, err
	}
...
func (suite *E2ETestSuite) TestErrorHandling() {
	...
	suite.Run("Invalid JSON", func() {
		req, _ := http.NewRequestWithContext(context.Background(), "POST", suite.baseURL+"/api/v1/crypto/hash",
