...
func TestStartHTTPSpan(t *testing.T) {
	...
	ctx := context.Background()
	_, span := tracer.StartHTTPSpan(ctx, "GET", "/test")
	assert.NotNil(t, span)
	assert.True(t, span.SpanContext().IsValid())
	span.End()
}
...
func TestStartCryptoSpan(t *testing.T) {
	...
	ctx := context.Background()
	_, span := tracer.StartCryptoSpan(ctx, "hash", "sha256")
	assert.NotNil(t, span)
	assert.True(t, span.SpanContext().IsValid())
	span.End()
}
...
func TestStartTextSpan(t *testing.T) {
	...
	ctx := context.Background()
	_, span := tracer.StartTextSpan(ctx, "case_convert")
	assert.NotNil(t, span)
	assert.True(t, span.SpanContext().IsValid())
	span.End()
}
...
func TestStartTransformSpan(t *testing.T) {
	...
	ctx := context.Background()
	_, span := tracer.StartTransformSpan(ctx, "encode", "base64")
	assert.NotNil(t, span)
	assert.True(t, span.SpanContext().IsValid())
	span.End()
}
...
func TestStartNetworkSpan(t *testing.T) {
	...
	ctx := context.Background()
	_, span := tracer.StartNetworkSpan(ctx, "dns_lookup", "example.com")
	assert.NotNil(t, span)
	assert.True(t, span.SpanContext().IsValid())
	span.End()
}
...
func BenchmarkTraceFunction(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_ = tracer.TraceFunction(ctx, "benchmark-function", func(ctx context.Context) error {
			return nil
		})
	}
}
